...
**Klex is environment[/runtime]-agnostic:** By default, Klex only has access to one dedicated, isolated `data-directory` on the machine it's running on. Every other connection to outside environments and machines is established via MCP and an MCP-Extension:
...
**Klex is self-improving:** After every task, Klex will reflect on its actions - and update its internal understanding and memory to do the task more efficiently the next time.


**Klex is sessionless:** There is no dedicated policy that determines which session an incoming message belongs to - every message is sent into the brain of Klex, where an agentic router decides what to do with the message:
...
**Klex is model-aware:** Klex knows about its available models and their tradeoffs:
