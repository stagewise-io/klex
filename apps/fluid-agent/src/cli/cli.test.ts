...
describe('parseCliArgs', () => {
  const originalEnv = process.env.FLUID_DATA_DIR;
  const originalCwd = process.cwd();

  beforeEach(() => {
    delete process.env.FLUID_DATA_DIR;
  });

  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env.FLUID_DATA_DIR = originalEnv;
    } else {
      delete process.env.FLUID_DATA_DIR;
    }
  });
  ...
  it('uses FLUID_DATA_DIR env var when no CLI arg provided', () => {
    process.env.FLUID_DATA_DIR = '/env/data-dir';
    const result = parseCliArgs([]);
    expect(result.dataDirectory).toBe('/env/data-dir');
  });

  it('CLI arg overrides FLUID_DATA_DIR env var', () => {
    process.env.FLUID_DATA_DIR = '/env/data-dir';
