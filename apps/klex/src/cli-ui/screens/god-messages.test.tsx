...
describe('GodMessagesScreen', () => {
  ...
  it('blocks interaction while a reset is pending', async () => {
    const pendingReset = deferred<{ sessionId: string }>();
    const resetGodSession = vi.fn().mockReturnValue(pendingReset.promise);
    const sendGodMessage = vi.fn();
    const view = renderScreen(makeClient({ resetGodSession, sendGodMessage }));
    await vi.waitFor(() => expect(view.lastFrame()).toContain('IDLE'));
    ...
    view.stdin.write('y');
    await vi.waitFor(() =>
      expect(view.lastFrame()).toContain('Resetting session'),
    );

    view.stdin.write('r');
    view.stdin.write('\r');
    await new Promise((resolve) => setTimeout(resolve, 25));
    expect(resetGodSession).toHaveBeenCalledOnce();
    expect(sendGodMessage).not.toHaveBeenCalled();

    pendingReset.resolve({ sessionId: 'fresh-session' });
    await vi.waitFor(() =>
      expect(view.lastFrame()).not.toContain('Resetting session'),
    );
    view.unmount();
  });
