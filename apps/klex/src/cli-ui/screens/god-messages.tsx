...
type Mode = 'overview' | 'compose' | 'sending' | 'confirm-reset' | 'resetting';
...
export function GodMessagesScreen({
  ...
  useMenuInput({
    [MenuKeys.Back]: () => {
      if (modeRef.current === 'sending' || modeRef.current === 'resetting') {
        return;
      }
