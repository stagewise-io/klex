...
{
  "configVersion": 2,
  "episodeFinishIdleTriggerTimeMs": 60000,
  "memoryWriteIntervalMs": 120000,
...
It defaults to `120000` (two minutes).

`memoryWriteStepInterval` sends pending history after that many completed
main-session steps since the previous successful write. It defaults to `10`.
...
episode it writes any pending history. It defaults to `60000` (one minute).
