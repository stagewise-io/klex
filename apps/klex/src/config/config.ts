...
class ConfigModule implements Config {
  ...
  resolveRealtimeProvider(): ResolvedRealtimeProvider | undefined {
    ...
    if (!apiKey) {
      throw new ConfigValidationError(
        `Realtime ${resolved.providerType === 'openai' ? 'OpenAI' : 'Google Gemini'} provider requires an API key`,
      );
    }
    const model = {
    ...
    if (resolved.providerType === 'google-gemini') {
      if (!isGeminiLiveModelId(resolved.modelId))
        throw new ConfigValidationError(
          `Unsupported Google Gemini realtime model '${resolved.modelId}'`,
          { code: 'type_mismatch' },
        );
      return {
        kind: 'gemini-live',
        ...
        config: {
          ...
          ...(resolved.modelId === 'gemini-3.8-live-extended-thinking' && {
            thinkingLevel: resolveGeminiThinkingLevel(resolved.providerOptions),
          }),
        },
      };
    }
...
function isGeminiLiveModelId(modelId: string): modelId is GeminiLiveModelId {
  return (
...
function resolveGeminiThinkingLevel(
  ...
  const live = google?.live;
  if (live === undefined) return 'medium';
  if (typeof live !== 'object' || live === null || Array.isArray(live))
    throw new ConfigValidationError(
      'Google Gemini live provider options must be an object',
    );
  const thinkingLevel = (live as Record<string, unknown>).thinkingLevel;
  if (thinkingLevel === undefined) return 'medium';
  if (
    thinkingLevel !== 'low' &&
    ...
    throw new ConfigValidationError(
      "Google Gemini Live thinkingLevel must be 'low', 'medium', or 'high'",
    );
