...
function createCloud(
  ...
  return {
    ...
    isCloudEnabled: () => availability.enabled,
...
describe('MCP OAuth detection', () => {
  ...
  it.each([
    { enabled: false, enrolled: true },
    { enabled: true, enrolled: false },
  ])(
    'leaves OAuth to the regular transport when Cloud availability is $enabled/$enrolled',
    ({ enabled, enrolled }) => {
      const cloud = createCloud(['unused'], { enabled, enrolled });
...
describe('MCP protected-resource discovery', () => {
  ...
  it('caches discovery, refreshes once after 401, and never loops', async () => {
    ...
    expect(cloud.invalidateAccessToken).toHaveBeenCalledOnce();
