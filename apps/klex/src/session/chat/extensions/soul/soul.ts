...
class SoulExt implements Extension {
  ...
  private containsOfficialName(content: string): boolean {
    const officialName = this.deps.config.get().officialName.trim();
    return content
      .normalize('NFKC')
      .toLocaleLowerCase()
      .includes(officialName.normalize('NFKC').toLocaleLowerCase());
  }
  ...
  private soulContentSchema() {
    return z
      ...
      .min(1)
      .max(MAX_SOUL_LENGTH)
      .refine((content) => !this.containsOfficialName(content), {
        message: 'Soul content must not include the official name',
      });
  }

  private officialNameError(content: string): string | null {
    return this.containsOfficialName(content)
      ? 'Soul content must not include your official name.'
      : null;
  }
...
- Calibration — one example only if it fixes a recurring failure.`,
          inputSchema: z.object({
            content: this.soulContentSchema().describe(
          ...
          execute: async ({ content }) => {
            const officialNameError = this.officialNameError(content);
            if (officialNameError) return officialNameError;
