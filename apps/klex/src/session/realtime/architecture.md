...
Responses delegation stays provider-driven: Klex does not open a separate Responses request. The adapter correlates the outer delegation ID, nested response ID, and stable function `call_id`. Only completed function-call items become provider-neutral tool events. The coordinator remains the sole authority for validation and execution through the interaction lease. Structured outputs return to the matching call. For a completed response containing tool calls, one continuation is sent only after every call settles.
