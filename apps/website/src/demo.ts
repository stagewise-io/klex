import { botAvatarMarkup } from './bot-avatar';

const mark = (name: string, label: string) =>
  `<img src="/connectors/${name}.svg" alt="" width="22" height="22" />${label}`;

export function examplesFor(name: 'Harry' | 'Sarah') {
  const bot = () =>
    `<span class="avatar bot-avatar" aria-hidden="true">${botAvatarMarkup(name)}</span>`;
  const identity = `<strong>${name}</strong> <span class="bot-label">BOT</span>`;
  return [
    {
      id: 'slack',
      label: mark('slack', 'Slack'),
      content: `<div class="slack-layout">
      <aside class="slack-sidebar" aria-label="Example Slack workspace"><strong>Acme workspace</strong><p>Channels</p><span># general</span><span class="channel-selected"># launch</span><span># engineering</span><p>Direct messages</p><span>${name} <small>BOT</small></span></aside>
      <div class="slack-conversation"><div class="native-toolbar"><strong># launch</strong><span>Launch planning</span></div>
        <div class="date-divider">Today</div>
        <div class="message"><span class="avatar human-avatar" aria-hidden="true">JL</span><div><strong>Jamie Lee</strong> <time>9:41 AM</time><p><span class="mention">@${name}</span> Can you turn yesterday’s launch notes into a checklist for the team?</p></div></div>
        ...
        <div class="message"><span class="avatar human-avatar" aria-hidden="true">JL</span><div><strong>Jamie Lee</strong> <time>9:44 AM</time><p>I’ll take the email. Thanks, ${name}!</p></div></div>
