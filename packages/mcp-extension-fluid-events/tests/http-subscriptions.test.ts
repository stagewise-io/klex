...
function listenRequest(
  ...
  return new Request('http://localhost/mcp', {
    ...
    body: JSON.stringify({
      ...
      params: {
        notifications: {
          [FLUID_EVENTS_EXTENSION_ID]: options.subscription ?? {
            afterCursor: '0',
          },
        },
        _meta: metadata,
...
describe('Fluid Events HTTP subscriptions', () => {
  it('delegates unrelated requests unchanged', async () => {
    const delegate = vi.fn(async () => new Response('delegated'));
    const manager = createFluidEventsHttpSubscriptionManager(delegate);
  ...
  it('rejects invalid filters and missing capabilities', async () => {
    const manager = createFluidEventsHttpSubscriptionManager(async () =>
      Response.json({}),
    );
