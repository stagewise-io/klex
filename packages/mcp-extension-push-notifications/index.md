---
title: Overview
description: Push Notifications gives MCP clients a durable event feed with optional low-latency notifications.
---

# Overview

Push Notifications is an extension to the Model Context Protocol. It gives an MCP
...
whether a message was stored, processed, or lost during a disconnect. Push
Notifications adds the small amount of state needed for reliable delivery:
...
sequenceDiagram
    ...
    Client->>Server: io.stagewise/push-notifications/get
    ...
    Client->>Server: io.stagewise/push-notifications/ack
    Server-->>Client: Empty result

    Note over Client,Server: While subscribed
    Server-->>Client: io.stagewise/push-notifications/event
```
...
Both peers declare `io.stagewise/push-notifications` in their MCP extension
