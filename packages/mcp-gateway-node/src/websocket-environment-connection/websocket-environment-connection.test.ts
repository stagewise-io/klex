...
describe('WebSocketEnvironmentConnection', () => {
  it('decodes frames in order and closes once', async () => {
    ...
    for (const type of ['session.opened', 'session.close'] as const)
      active.client.send(encodeGatewayFrame({ version: 1, type, sessionId }));
    await vi.waitFor(() =>
      expect(frames).toEqual(['session.opened', 'session.close']),
    );
