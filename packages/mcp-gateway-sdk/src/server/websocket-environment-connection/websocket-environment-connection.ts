  ...
  implements WebSocketEnvironmentConnection
{
  ...
  readonly #frameHandlers = new Set<
    (frame: EnvironmentToGatewayFrame) => void
  >();
    ...
    const operation = this.#sendChain.then(
      () =>
        new Promise<void>((resolve, reject) => {
          ...
          this.#socket.send(encodeGatewayFrame(frame), (error) => {
  ...
  readonly #handleMessage = (data: RawData, isBinary: boolean): void => {
    if (this.#closed) return;
    try {
      if (isBinary) throw new Error('Binary gateway frames are unsupported');
      const frame = decodeEnvironmentToGatewayFrame(data.toString('utf8'));
  ...
  #fail(cause: Error): void {
    if (this.#closed) return;
    this.#finish(cause);
    this.#socket.close(1002, 'Invalid gateway frame');
  }
