...
export interface ProxyDaemonConnection {
  readonly url: URL | string;
  readonly headers?: Readonly<Record<string, string>>;
}
export type ProxyDaemonConnectionProvider = () =>
  | ProxyDaemonConnection
  | Promise<ProxyDaemonConnection>;
export interface ProxyDaemonReconnectOptions {
  readonly initialDelayMs?: number;
  readonly maximumDelayMs?: number;
  readonly factor?: number;
  readonly jitter?: number;
}
export interface ProxyEnvironmentHandler {
  fetch(request: Request): Promise<Response>;
  close(): Promise<void>;
}
export type ProxyDaemonState =
  | 'idle'
  | 'connecting'
  | 'connected'
  | 'reconnecting'
...
export interface ProxyDaemon {
  start(): Promise<void>;
  close(): Promise<void>;
  readonly state: ProxyDaemonState;
...
class ProxyDaemonModule implements ProxyDaemon {
  ...
  constructor(options: ProxyDaemonOptions) {
  ...
  async #connect(): Promise<void> {
    ...
    if (url.protocol !== 'ws:' && url.protocol !== 'wss:') {
      throw new TypeError('Proxy URL must use ws: or wss:');
    }
    if (this.#closed) throw new Error('Daemon is closed');
  ...
  async #dispatch(data: RawData, isBinary: boolean): Promise<void> {
    ...
    const frame = decodeProxyToEnvironmentFrame(data.toString('utf8'));
  ...
  async #serve(
    frame: Extract<
      ReturnType<typeof decodeProxyToEnvironmentFrame>,
      { type: 'exchange.open' }
    >,
        ...
        while (this.#exchanges.has(frame.exchangeId)) {
          ...
          for (
            ...
            await this.#send({
              ...
              exchangeId: frame.exchangeId,
              data: bytesToBase64(
                result.value.subarray(offset, offset + MAX_PROXY_CHUNK_BYTES),
              ),
            });
          }
        }
  ...
  #send(frame: EnvironmentToProxyFrame): Promise<void> {
    const operation = this.#sendChain.then(
      () =>
        new Promise<void>((resolve, reject) => {
          const socket = this.#socket;
          if (!socket || socket.readyState !== WebSocket.OPEN) {
            reject(new Error('Proxy connection is closed'));
            return;
          }
          socket.send(encodeProxyFrame(frame), (error) =>
...
export function createProxyDaemon(options: ProxyDaemonOptions): ProxyDaemon {
  return new ProxyDaemonModule(options);
}
function normalizeReconnect(
  reconnect: ProxyDaemonOptions['reconnect'],
